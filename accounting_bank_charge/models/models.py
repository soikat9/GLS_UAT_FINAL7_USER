...
class AccountPayment(models.Model):
    ...
    def _synchronize_from_moves(self, changed_fields):
        for line in self:
            if line._context.get('skip_account_move_synchronization'):
                return

            for pay in line.with_context(skip_account_move_synchronization=True):
                ...
                if 'line_ids' in changed_fields:
                    ...
                    writeoff_lines = writeoff_lines.filtered(
                        lambda e: e.bank_charge_line is False and e.bank_tax_charge_line is False and e.additional_cost_line is False)
                    if writeoff_lines and not writeoff_lines.bank_charge_line and not writeoff_lines.bank_tax_charge_line and not writeoff_lines.additional_cost_line and len(
                    ...
                    if len(counterpart_lines) > 1:
                        for counter in counterpart_lines:
                            payment_vals_to_write.update({
                                'amount': abs(liquidity_amount),
                                'partner_type': partner_type,
                                'currency_id': liquidity_lines.currency_id.id,
                                'destination_account_id': counter.account_id.id,
                                'partner_id': liquidity_lines.partner_id.id,
                            })
                    else:
                        payment_vals_to_write.update({
                ...
                if line.is_bank_charges and not move.line_ids.filtered(lambda e: e.bank_charge_line is True):
                    if line.is_additional_cost and not move.line_ids.filtered(lambda e: e.additional_cost_line is True):
                        # if line.journal_type == "sale":
                            move.write({'line_ids': [
                                (0, 0, {
                                    "name": 'Bank Charges',
                                    "ref": line.ref,
                                ...
                                (0, 0, {
                                    ...
                                    "partner_id": line.partner_id.id or False,
                                    "journal_id": line.journal_id.id,
                                    "account_id": line.bank_charges_account.id,
                                    "debit": line.bank_charges,
                                    "credit": 0.0,
                                    "date_maturity": line.date,
                                    "bank_charge_line": True,
                                    "additional_cost_line": True
                                ...
                                (0, 0, {
                                    ...
                                    "partner_id": line.partner_id.id or False,
                                    "journal_id": line.journal_id.id,
                                    "account_id": line.additional_cost_account.id,
                        ...
                        if line.journal_type == "sale":
                            move.write({'line_ids': [
                                (0, 0, {
                                    "name": 'Additional Cost/Bank Charges',
                                    "ref": line.ref,
                                    'currency_id': line.currency_id.id,
                                    "partner_id": line.partner_id.id or False,
                                    "journal_id": line.journal_id.id,
                    ...
                    if line.is_bank_charges and line.is_bank_tax_applicable and not move.line_ids.filtered(
                            lambda e: e.bank_tax_charge_line is True):
                        taxes = line.get_tax_vals()
                        tax_line = []
                        for tax in line.get_tax_vals():
                            tax_line += (0, 0, {
                                    ...
                                    "debit": line.bank_tax_amount if line.payment_type == 'outbound' else 0.0,
                                    "credit": line.bank_tax_amount if line.payment_type == 'inbound' else 0.0,
                                    "date_maturity": line.date,
                        ...
                        move.write({'line_ids': [
                            (0, 0, {
                                ...
                                "journal_id": line.journal_id.id,
                                "account_id": line.journal_id.default_account_id.id,
                                "debit": line.bank_tax_amount if line.payment_type == 'inbound' else 0.0,
                                "credit": line.bank_tax_amount if line.payment_type == 'outbound' else 0.0,
                                "date_maturity": line.date,
                                "bank_tax_charge_line": True
                            }),
                            tax_line,
